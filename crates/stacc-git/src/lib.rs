...
impl Git {
    ...
    /// Stage every change like [`stage_all`], then drop from the index any
    /// tracked path the now-staged ignore rules cover, leaving the working tree
    /// untouched.
    ///
    ...
    /// Tracked files that the current ignore rules (including unstaged
    /// `.gitignore` edits) now cover. `-c core.quotePath=false` keeps non-ASCII
    /// paths unquoted so they round-trip back into `git rm`.
