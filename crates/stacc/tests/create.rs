...
fn create_with_nothing_staged_makes_an_empty_tracked_branch() {
    ...
    let out = stacc(p, &["create", "feat-y", "--json"]);
    assert!(
        out.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&out.stderr)
...
fn create_uninitialized_errors() {
    ...
    let out = stacc(p, &["create", "feat-z", "--json"]);
...
fn create_from_detached_head_errors() {
    ...
    let out = stacc(p, &["create", "feat-d", "--json"]);
    assert!(!out.status.success());
    assert!(
...
fn create_refuses_an_already_tracked_name() {
    ...
    let out = stacc(p, &["create", "dup", "--json"]);
    assert!(!out.status.success());
    assert!(
...
fn create_refuses_the_trunk_name() {
    let tmp = init_repo();
    let p = tmp.path();
    let out = stacc(p, &["create", "main", "--json"]);
    assert!(!out.status.success());
    assert!(
...
fn create_on_an_existing_git_branch_fails_without_partial_state() {
    ...
    let out = stacc(p, &["create", "taken", "--json"]);
    assert!(!out.status.success());
...
fn create_all_drops_a_path_the_commit_adds_to_gitignore() {
    ...
    let out = stacc(p, &["create", "feat-ig", "-a", "-m", "ignore cache", "--json"]);
    assert!(
        out.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&out.stderr)
...
fn create_all_keeps_a_force_added_file_when_gitignore_is_unchanged() {
    ...
    let out = stacc(p, &["create", "feat-x", "-a", "-m", "more", "--json"]);
    assert!(
        out.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&out.stderr)
...
fn create_onto_bases_on_the_named_branch() {
    ...
    let parent = stacc(p, &["parent", "--json"]);
    assert!(
...
fn create_insert_reparents_children_onto_the_new_branch() {
    ...
    run_git(p, &["add", "n.txt"]);
    let out = stacc(p, &["create", "n", "--insert", "-m", "n1", "--json"]);
    assert!(
        out.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&out.stderr)
...
fn create_insert_with_onto_errors() {
    let tmp = init_repo();
    let p = tmp.path();
    let out = stacc(p, &["create", "x", "--insert", "--onto", "main", "--json"]);
    assert!(!out.status.success());
    assert!(
...
fn create_handles_slashed_branch_names() {
    ...
    std::fs::write(p.join("f.txt"), "hi\n").expect("write");
    run_git(p, &["add", "f.txt"]);
    let out = stacc(p, &["create", "jillian/feat", "--json"]);
    assert!(
        out.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&out.stderr)
